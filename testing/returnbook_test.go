...
func TestReturnBook(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedBody:   DataSet["valid_return_book"],
		},
		{
			name:           "Invalid Loan Id",
			requestBody:    map[string]uint{"loan_id": 2},
			expectedStatus: http.StatusInternalServerError,
			expectedBody:   DataSet["invalid_return_book"],
		},
		{
			name:           "Empty loan id",
			requestBody:    map[string]uint{},
...
func MockReturnBook(loan_id int) (MockedBookStoreResponse, error) {
	if loan_id == 1 {
		...
		}, fmt.Errorf("failed to lock and fetch loan: record not found")
	}
...
func ReturnBook(c *gin.Context) {
	...
	if res, err := MockReturnBook(int(reqbody.LoanId)); err == nil {
