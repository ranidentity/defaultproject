...
func (ref *BookStoreRepository) LoanBook(book_id uint, new_count int, from time.Time, to time.Time, name_of_borrower string) (new model.LoanDetail, err error) {
	tx := model.DB.Begin()
	...
	if insert_error := tx.Debug().Table("loan_detail").Create(&new).Error; insert_error != nil {
		tx.Rollback()
		return model.LoanDetail{}, fmt.Errorf("failed to initiate loan: %v", err)
	}
	update_book := tx.Model(ref.Book).
		Where("ID = ?", book_id).
		Where("available_copies > 0").
		Update("available_copies", new_count)
	if update_book.Error != nil {
		tx.Rollback()
		return model.LoanDetail{}, fmt.Errorf("failed to update book's availability: %v", err)
	}
	// else {
	// 	affected_row = int(update_book.RowsAffected)
	// }
	err = tx.Commit().Error
	return
}
func (ref *BookStoreRepository) ExtendLoan(loan_id uint) (affected_row int, err error) {
	...
	if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).
		Where("id = ?", loan_id).
		Where("book_returned_on IS NULL").
